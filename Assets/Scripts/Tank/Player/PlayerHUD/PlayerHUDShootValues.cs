...
public class PlayerHUDShootValues : MonoBehaviour
{
    ...
    protected virtual void Awake() => _shootController = Get<ShootController>.From(gameObject);
    ...
    {
        if (_angleText != null) 
            _angleText.text = Mathf.Round(Mathf.InverseLerp(hudValues._minAngle, hudValues._maxAngle, hudValues._currentAngle) * 100) + "°";

        if (_forceText != null) 
            _forceText.text = Mathf.Round(Mathf.InverseLerp(hudValues._minForce, hudValues._maxForce, hudValues._currentForce) * 100).ToString();
