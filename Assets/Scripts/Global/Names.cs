

public static class Names 
{
    public static string HorizontalJoystick = "HorizontalJoystick";
    public static string VerticalJoystick = "VerticalJoystick";

    public static string L { get; private set; } = "L";
    public static string LS { get; private set; } = "LS";
    public static string M { get; private set; } = "M";
    public static string R { get; private set; } = "R";
    public static string RL { get; private set; } = "RL";
    public static string RS { get; private set; } = "RS";
    public static string RTL { get; private set; } = "RTL";
    public static string T { get; private set; } = "T";
    public static string TL { get; private set; } = "TL";
    public static string TR { get; private set; } = "TR";

    public static string Play { get; private set; } = "play";

    public static string Tank_FirstPlayer { get; private set; } = "Tank1";
    public static string Tank_SecondPlayer { get; private set; } = "Tank2";

    public static string AirSupport { get; private set; } = "Air Support";
    public static string Sandbags { get; private set; } = "Sandbags";
    public static string MetalCube { get; private set; } = "Metal Cube";
    public static string MetalGround { get; private set; } = "Metal Ground";
    public static string Shield { get; private set; } = "Shield";
    public static string LightMortarSupport { get; private set; } = "60mm Mortar Support";
}
