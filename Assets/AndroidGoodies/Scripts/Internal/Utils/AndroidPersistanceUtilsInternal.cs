...
namespace DeadMosquito.AndroidGoodies.Internal
{
	...
	public static class AndroidPersistanceUtilsInternal
	{
		...
		{
			var filePath = Path.Combine(AGUtils.ExternalCacheDirectory.CallStr("getAbsolutePath"), GoodiesShareImageFileName);
			if (Debug.isDebugBuild)
			{
				Debug.Log("Android Goodies ----- path to temporary file: " + filePath);
			}
			...
			try
			{
				...
				file.Close();
				return GetUriFromFilePath(filePath);
			}
			catch (Exception e)
			{
		...
		static string SaveFileToExternalStorage(byte[] buffer, string fileName, string directory = null)
		{
