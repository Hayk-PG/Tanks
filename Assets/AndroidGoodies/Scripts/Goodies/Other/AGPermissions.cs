...
namespace DeadMosquito.AndroidGoodies
{
	...
	public static class AGPermissions
	{
		...
		/// <summary>
		/// Helper method to request runtime permissions before executing the <see cref="Action"/> if the permission is not granted yet.
		/// </summary>
		/// <param name="permission">Permission that certain method requires</param>
		/// <param name="action">Method to execute</param>
		/// <param name="onPermissionDenied">Executed if user denies the permission.</param>
		[PublicAPI]
		public static void ExecuteIfHasPermission(string permission, [NotNull] Action action, [NotNull] Action onPermissionDenied)
		{
			if (action == null)
			{
			...
			{
				throw new ArgumentNullException("onPermissionDenied");
			}
			...
			{
				RequestPermissions(new[] {permission}, results =>
				{
					if (results[0].Status == PermissionStatus.Granted)
					{
						action();
					}
					else
